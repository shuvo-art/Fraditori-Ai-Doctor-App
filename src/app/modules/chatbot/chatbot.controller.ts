...
import TextAgent from './chatbot.utils';
...
// Controller to handle getting a bot reply with summary context
export const handleGetBotReply: RequestHandler = async (req, res) => {
  try {
    ...
    // Fetch chat history and generate a summary
    ...
    // Include the summary as part of the system message context
    const botReply = await agent.getDoctorAssistResponse(prompt, summary);

    // Respond to the client with the bot reply
    res.status(200).json({ success: true, response: botReply });
  } catch (error: any) {
    console.error("Error in handleGetBotReply:", error);
    res.status(500).json({ success: false, message: error.message || "Failed to get a bot reply." });
  }
};
...
export const handleChatMessage: RequestHandler = async (req, res) => {
  try {
    ...
    const summary = await agent.summarizeChatHistory(chatHistory._id.toString());
    ...
    // Push the bot's response to chat history
    chatHistory.chat_contents.push({
      id: botMessageId,
      sent_by: "Bot",
    ...
    // Save the updated chat history to the database (with the bot's response)
    await chatHistory.save();
